...
HARNESS_ENV_FILENAME = ".harness-env.sh"


def harness_env_vars(main_root: Path | None = None) -> dict[str, str]:
    """Env vars that protect the main venv from worktree-side `uv run` rewrites.

    The worktree carries a full checkout (including `pyproject.toml`). Without
    these vars, `uv run pytest|ruff|mypy` inside the worktree treats the
    worktree as the project root and auto-syncs the `.venv` symlink — which
    rewrites the editable `.pth` and console-script shebangs to the worktree
    path. After the task merges and the worktree is cleaned up, the rewritten
    `dimos` / `pytest` / etc. entry points have dead shebangs.

    Setting `UV_NO_SYNC=1` disables that implicit sync; setting
    `UV_PROJECT_ENVIRONMENT` pins the venv path so uv never falls back to
    creating a fresh one inside the worktree.
    """
    root = main_root if main_root is not None else _main_repo_root()
    venv = root / ".venv"
    return {
        "UV_NO_SYNC": "1",
...
def _write_harness_env_file(worktree: Path) -> None:
    """Drop a sourceable env file into the worktree.

    Subagents (implementer, reviewer, babysitter) source this at workflow
    start so every `uv` / `pre-commit` invocation downstream inherits the
    protection vars without each prompt having to recite them.
    """
    main_root = _main_repo_root()
    venv = main_root / ".venv"
    body = (
        "# Auto-generated by dimos-agentic-harness/scripts/worktree.py.\n"
        "# Source this before running any `uv` / `pre-commit` command inside\n"
        "# this worktree, so they cannot rewrite the main repo's .venv.\n"
        f"export UV_NO_SYNC=1\n"
        f'export UV_PROJECT_ENVIRONMENT="{venv}"\n'
    )
    target = worktree / HARNESS_ENV_FILENAME
    try:
...
def _no_lfs_env() -> dict[str, str]:
    """Env that skips LFS smudge AND blocks worktree-side `uv` auto-sync.

    LFS files materialize as pointer text in the worktree (no GB downloads).
    The `UV_*` vars protect the main `.venv` from being mutated by an
    implicit `uv sync` when subagents run `uv run` inside the worktree —
    see `harness_env_vars` for the failure mode this guards against.
    """
    env = os.environ.copy()
...
def reset_worktree(task_id: str, branch: str, base_ref: str = "origin/dev") -> Path:
    ...
    subprocess.run(
        ["git", "clean", "-fdx", "-e", ".venv", "-e", HARNESS_ENV_FILENAME],
        cwd=wt,
        check=False,
    )
    _venv_symlink(wt)
