...
"""Install dimos-pr-flow git hooks into .git/hooks/.

Hooks live as source under `<this-skill>/hooks/` (resolved relative to
this file) and are symlinked into `.git/hooks/` so updates flow through
automatically. Worktrees share the same .git directory, so hooks apply
to all of them.
...
take over. (The `harness-backup` prefix is shared with `dimos-agentic-
harness` so the two skills' pre-push hooks can chain to the same
pre-existing hook, e.g. `git lfs install`'s.) Existing symlinks pointing
into our hooks dir are reused.
"""
