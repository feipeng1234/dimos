...
Unit tests fabricate synthetic step responses with known K/tau/L and
...
def test_fopdt_recovers_under_noise_within_5pct() -> None:
    ...
    t, y = _synthetic_step(K_true, tau_true, L_true, u_step, noise_std=0.01, seed=42)
...
def test_aggregate_group_inverse_variance_weighted_mean() -> None:
    fits = [_mk_runfit(f"r{i}", K=1.0 + 0.01 * i, tau=0.2, L=0.05) for i in range(5)]
...
def test_select_edge_projects_params_down_onto_params() -> None:
    ...
    rf = RunFit(
        ...
        split="train",
        params=rise_params,
        params_down=fall_params,
        extra={"baseline": 0.0},
        extra_down={"baseline_down": 1.0},
    )
...
def test_compare_rise_fall_flags_decel_slower_than_accel() -> None:
    ...
    # Fabricate a "fall" summary where tau is 2x the rise - that's a real
    ...
    # ratio fall/rise should be ~2 since fall tau = 2x rise tau.
...
def test_run_fit_smoke_includes_params_down(real_session: Path) -> None:
    ...
    from dimos.utils.characterization.modeling.fit import fit_run

    step_runs = []
    for rd in sorted(
        p for p in real_session.iterdir() if p.is_dir() and p.name and p.name[0].isdigit()
    ):
        rj = rd / "run.json"
        if not rj.exists():
            continue
...
def test_fit_session_smoke_real_session(tmp_path: Path, real_session: Path) -> None:
    ...
    from dimos.utils.characterization.modeling.fit import fit_session
