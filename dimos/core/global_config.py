...
class GlobalConfig(BaseSettings):
    ...
    # Whether mujoco_process skips the interactive viewer and runs the
    # physics loop offscreen.  ``None`` (default) means auto-detect from
    # the environment (no $DISPLAY on Linux → headless).  ``True`` /
    # ``False`` are explicit overrides.  Required on macOS in non-GUI
