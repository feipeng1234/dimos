...
class BaselinePathFollowerTaskConfig:
    ...
    k_angular: float = 0.5
    # Optional inner-loop velocity-tracking PID. None ⟹ pure feedforward
    # (PController output goes straight to adapter).
...
class BaselinePathFollowerTask(BaseControlTask):
    ...
    def __init__(
        ...
        self._controller = PController(
            global_config, config.speed, config.control_frequency
        )
    ...
    def compute(self, state: CoordinatorState) -> JointCommandOutput | None:
        ...
        # Optional inner-loop PI velocity tracking — corrects desired against
        # measured (plant-output) velocities to compensate for FOPDT gain/lag.
    ...
    def on_preempted(self, by_task: str, joints: frozenset[str]) -> None:
        if joints & self._joint_names and self.is_active():
            logger.warning(
                f"BaselinePathFollowerTask '{self._name}' preempted by {by_task}"
            )
            self._state = "aborted"
    ...
    def _step_path_following(self) -> tuple[float, float, float]:
        ...
        closest = self._distancer.find_closest_point_index(pos)
